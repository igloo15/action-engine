...
export class ActionEngine<T> {
  ...
  addAction(action: ActionService<T>, interval: number = 1, name: string = '', range: number = 0) {
    this.addInternalAction(action, interval, range, name, 'normal');
  }

  addPreciseAction(action: ActionService<T>, interval?: number, range?: number, name?: string) {
    this.addInternalAction(action, interval, range, name, 'precise');
  }

  addAnimationAction(action: ActionService<T>, interval?: number, range?: number, name?: string) {
    this.addInternalAction(action, interval, range, name, 'normalAnim');
  }

  addPreciseAnimationAction(action: ActionService<T>, interval?: number, range?: number, name?: string) {
  ...
  private validateInterval(name: string, interval: number) {
    ...
    const loopCount = this.leastCommonMultiple(this._interval, interval) / interval;
